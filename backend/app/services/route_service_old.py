...
class RouteService:
    def __init__(self):
        ...
        self.rotas_brasil_service = RotasBrasilService()  # Novo serviço principal
    ...
    # Função removida - a API de autocomplete do Rotas Brasil deve ser usada apenas 
    # para encontrar uma cidade específica, não para buscar sugestões genéricas
    ...
    async def calcular_pedagio(self, loads: List[List]) -> float:
            ...
            print(f"Buscando coordenadas na API do Rotas Brasil...")
            
            # Usar o RotasBrasilService para buscar coordenadas
            try:
                # Extrair estado se disponível
                estado = None
                if ', ' in cidade_normalizada:
                    partes = cidade_normalizada.split(', ')
                    cidade_nome = partes[0].strip()
                    if len(partes) > 1:
                        estado = partes[1].strip()
                else:
                    cidade_nome = cidade_normalizada.strip()
                
                # Buscar coordenadas usando a API do Rotas Brasil
                coords = await self.rotas_brasil_service.buscar_coordenadas_cidade(cidade_nome, estado)
                ...
                if coords:
                    ...
                    print(f"Coordenada encontrada via Rotas Brasil: {lat}, {lon}")
                    return lat, lon
                else:
                    print(f"Coordenada não encontrada na API do Rotas Brasil")
                    return None, None
                    
            except Exception as e:
                print(f"Erro ao buscar coordenadas na API do Rotas Brasil: {e}")
                return None, None
        except Exception as e:
            print(f"Erro ao obter coordenadas para {cidade}: {e}")
            return None, None
    ...
    async def processar_rota(self, origem: str, destino: str) -> Dict[str, Any]:
        """Processa uma rota individual - ROTAS BRASIL API COMO MÉTODO PRINCIPAL"""
        try:
            print(f"\n=== NOVA ROTA RECEBIDA ===")
            ...
            # MÉTODO PRINCIPAL: ROTAS BRASIL API
            print(f"Tentando Rotas Brasil API...")
            try:
                resultado = await self.rotas_brasil_service.processar_rota(origem, destino)
                
                # Verificar se a API foi bem-sucedida
                if resultado.get("distance", 0) > 0:
                    print(f"Rotas Brasil API bem-sucedida: {resultado['distance']}km, R${resultado['pedagios']}")
                    ...
                    self.cache_rotas[chave_rota] = {
                        ...
                        'timestamp': time.time(),
                        'validado': True,
                        'metodo': 'rotas_brasil_api'
                    }
                    
                    print(f"Resultado salvo no cache")
                    print(f"=== RESULTADO FINAL (ROTAS BRASIL API) ===")
                    print(f"{origem} -> {destino}")
                    print(f"Distancia: {resultado['distance']} km")
                    print(f"Pedagios: R$ {resultado['pedagios']}")
                    print(f"Rota processada com sucesso!\n")
                    return resultado
                else:
                    print(f"Rotas Brasil API falhou, tentando webscraping como fallback...")
                    raise Exception("Rotas Brasil API retornou resultado inválido")
                    
            except Exception as api_error:
                print(f"Erro na Rotas Brasil API: {api_error}")
                print(f"Tentando webscraping como fallback...")
                ...
                try:
                    ...
                    if resultado.get("distance", 0) > 0:
                        print(f"Webscraping fallback bem-sucedido: {resultado['distance']}km, R${resultado['pedagios']}")
                        ...
                        self.cache_rotas[chave_rota] = {
                            ...
                            'timestamp': time.time(),
                            'validado': True,
                            'metodo': 'webscraping_fallback'
                        }
                        
                        print(f"Resultado (fallback) salvo no cache")
                        print(f"=== RESULTADO FINAL (WEBSCRAPING FALLBACK) ===")
                        print(f"{origem} -> {destino}")
                        print(f"Distancia: {resultado['distance']} km")
                        print(f"Pedagios: R$ {resultado['pedagios']}")
                        print(f"Rota processada com sucesso!\n")
                        ...
                        raise Exception("Webscraping fallback também falhou")
                        
                except Exception as webscraping_error:
                    print(f"Erro no webscraping fallback: {webscraping_error}")
                    print(f"Tentando API original como último recurso...")
                    
                    # ÚLTIMO RECURSO: API ORIGINAL
                    resultado = await self._processar_rota_api_fallback(origem, destino)
                    
                    # Armazenar no cache marcando como último recurso
                    self.cache_rotas[chave_rota] = {
                        'resultado': resultado,
                        'timestamp': time.time(),
                        'validado': True,
                        'metodo': 'api_ultimo_recurso'
                    }
                    
                    print(f"Resultado (último recurso) salvo no cache")
                    print(f"=== RESULTADO FINAL (API ÚLTIMO RECURSO) ===")
                    print(f"{origem} -> {destino}")
                    print(f"Distancia: {resultado['distance']} km")
                    print(f"Pedagios: R$ {resultado['pedagios']}")
                    print(f"Rota processada com sucesso!\n")
    ...
    async def processar_rota_multipla(self, origem: str, destinos_lista: List[str]) -> Dict[str, Any]:
        """Processa uma rota com múltiplos destinos - ROTAS BRASIL API COMO MÉTODO PRINCIPAL"""
        try:
            destinos_str = ", ".join(destinos_lista)
            print(f"\n=== ROTA MULTIPLA RECEBIDA ===")
            print(f"Origem: {origem}")
            ...
            if chave_rota in self.cache_rotas:
                dados_cache = self.cache_rotas[chave_rota]
                if not self.cache_expirado(dados_cache['timestamp'], 6):  # 6 horas
                    print(f"Cache encontrado para rota multipla: {origem} -> {destinos_str}")
                    return dados_cache['resultado']
                else:
                    print(f"Cache expirado - removendo")
                    del self.cache_rotas[chave_rota]
            
            # MÉTODO PRINCIPAL: ROTAS BRASIL API MÚLTIPLA
            print(f"Tentando Rotas Brasil API multipla...")
            try:
                resultado = await self.rotas_brasil_service.processar_rota_multipla(origem, destinos_lista)
                
                # Verificar se a API foi bem-sucedida
                if resultado.get("total_distance", 0) > 0:
                    print(f"Rotas Brasil API multipla bem-sucedida: {resultado['total_distance']}km, R${resultado['total_pedagios']}")
                    ...
                    self.cache_rotas[chave_rota] = {
                        ...
                        'timestamp': time.time(),
                        'validado': True,
                        'metodo': 'rotas_brasil_api_multipla'
                    }
                    
                    print(f"Resultado multiplo salvo no cache")
                    print(f"=== RESULTADO FINAL (ROTAS BRASIL API MULTIPLA) ===")
                    print(f"{origem} -> {destinos_str}")
                    print(f"Distancia: {resultado['total_distance']} km")
                    print(f"Pedagios: R$ {resultado['total_pedagios']}")
                    print(f"Rota multipla processada com sucesso!\n")
                    return resultado
                else:
                    print(f"Rotas Brasil API multipla falhou, tentando webscraping como fallback...")
                    raise Exception("Rotas Brasil API multipla retornou resultado inválido")
                    
            except Exception as api_error:
                print(f"Erro na Rotas Brasil API multipla: {api_error}")
                print(f"Tentando webscraping como fallback...")
                
                # FALLBACK: WEBSCRAPING MÚLTIPLO
                try:
                    resultado = await self.webscraping_service.calcular_rota_multipla_webscraping(origem, destinos_lista)
                    
                    if resultado.get("distance", 0) > 0:
                        print(f"Webscraping multiplo fallback bem-sucedido: {resultado['distance']}km, R${resultado['pedagios']}")
                        ...
                        self.cache_rotas[chave_rota] = {
                            ...
                            'timestamp': time.time(),
                            'validado': True,
                            'metodo': 'webscraping_fallback_multiplo'
                        }
                        
                        print(f"Resultado multiplo (fallback) salvo no cache")
                        print(f"=== RESULTADO FINAL (WEBSCRAPING FALLBACK MULTIPLO) ===")
                        print(f"{origem} -> {destinos_str}")
                        print(f"Distancia: {resultado['distance']} km")
                        print(f"Pedagios: R$ {resultado['pedagios']}")
                        print(f"Rota multipla processada com sucesso!\n")
                        ...
                        raise Exception("Webscraping multiplo fallback também falhou")
                        
                except Exception as webscraping_error:
                    print(f"Erro no webscraping multiplo fallback: {webscraping_error}")
                    print(f"Tentando API original como último recurso...")
                    ...
                    self.cache_rotas[chave_rota] = {
                        ...
                        'timestamp': time.time(),
                        'validado': True,
                        'metodo': 'api_ultimo_recurso_multiplo'
                    }
                    
                    print(f"Resultado multiplo (último recurso) salvo no cache")
                    print(f"=== RESULTADO FINAL (API ÚLTIMO RECURSO MULTIPLO) ===")
                    print(f"{origem} -> {destinos_str}")
                    print(f"Distancia: {resultado['distance']} km")
                    print(f"Pedagios: R$ {resultado['pedagios']}")
                    print(f"Rota multipla processada com sucesso!\n")
            ...
            print(f"Erro geral ao processar rota multipla {origem} -> {destinos_lista}: {e}")
            return {
                "origem": origem,
