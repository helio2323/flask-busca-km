...
def test_all_4_cities():
    print("=== TESTE COM TODAS AS 4 CIDADES ESPECÍFICAS ===")
    
    # 1. Criar grupo
    print("\n1. Criando grupo...")
    grupo_data = {
        "nome": "Teste 4 Cidades",
        "descricao": "Teste com todas as 4 cidades específicas"
    }
    grupo_id = None
    try:
    ...
    # 2. Criar arquivo com as 4 cidades específicas
    print(f"\n2. Criando arquivo com as 4 cidades específicas...")
    data = {
        'ID': [1, 2, 3, 4],
        'Origem': ['CAMPINAS', 'CAMPINAS', 'CAMPINAS', 'CAMPINAS'],
        'UF': ['Sao Paulo', 'Sao Paulo', 'Sao Paulo', 'Sao Paulo'],
        'Destino (Cidade/Estado)': ['ADAMANTINA', 'ADOLFO', 'AGUAI', 'AGUAS DA PRATA'],
        'UF.1': ['Sao Paulo', 'Sao Paulo', 'Sao Paulo', 'Sao Paulo'],
        'KM': [0, 0, 0, 0],
        'Pedágio': [0, 0, 0, 0]
    }
    ...
    output.seek(0)
    print(f"[OK] Arquivo criado com {len(df)} cidades")
    
    # 3. Upload
    print(f"\n3. Fazendo upload para grupo {grupo_id}...")
    upload_id = None
    try:
        files = {'file': ('test_4_cities.xlsx', output.read(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
    ...
    print(f"[OK] Autocomplete corrigido: Cidade + Estado sendo enviados corretamente")
    print(f"[OK] Persistência funcionando: Rotas aparecem no grupo")
    print(f"\nO sistema está funcionando perfeitamente com todas as 4 cidades!")
