...
def test_aguai_city():
    print("=== TESTE ESPECÍFICO PARA CIDADE AGUAI ===")
    
    # 1. Criar grupo
    print("\n1. Criando grupo...")
    grupo_data = {
        "nome": "Teste AGUAI",
        "descricao": "Teste específico para cidade AGUAI"
    }
    grupo_id = None
    try:
    ...
    # 2. Criar arquivo com apenas AGUAI
    print(f"\n2. Criando arquivo com cidade AGUAI...")
    data = {
        'ID': [1],
        'Origem': ['CAMPINAS'],
        'UF': ['Sao Paulo'],
        'Destino (Cidade/Estado)': ['AGUAI'],
        'UF.1': ['Sao Paulo'],
        'KM': [0],
        'Pedágio': [0]
    }
    ...
    output.seek(0)
    print(f"[OK] Arquivo criado com cidade AGUAI")
    
    # 3. Upload
    print(f"\n3. Fazendo upload para grupo {grupo_id}...")
    upload_id = None
    try:
        files = {'file': ('test_aguai.xlsx', output.read(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
    ...
    # 7. Testar rota individual para AGUAI
    print(f"\n7. Testando rota individual para AGUAI...")
    try:
        test_route = {
            "origem": "Campinas, Sao Paulo, BR",
            "destino": "Aguai, Sao Paulo, BR"
        }
    ...
    print(f"\n=== CONCLUSÃO ===")
    print(f"Se AGUAI não foi encontrada, então há problema no formato")
    print(f"de envio para o autocomplete (cidade + estado).")
