...
export function RouteCalculator() {
  ...
  const calculateRoute = async () => {
    ...
    try {
      ...
      setResult({
        totalDistance: typeof response.distance === 'number' ? `${response.distance.toLocaleString()} km` : response.distance,
        totalTolls: typeof response.pedagios === 'number' ? `R$ ${response.pedagios.toFixed(2).replace('.', ',')}` : response.pedagios,
        estimatedTime: response.tempo_estimado ? `${Math.floor(response.tempo_estimado / 60)}h ${response.tempo_estimado % 60}min` : "N/A",
        fuelCost: response.combustivel_estimado ? `R$ ${response.combustivel_estimado.toFixed(2).replace('.', ',')}` : "N/A",
