...
def test_correct_position():
    print("=== TESTE COM POSIÇÃO CORRETA (8) ===")
    ...
    print("Usando a posição 8 (valor do meio) como valor do pedágio:")
    print(f"{'Nº':<3} {'Nome':<25} {'Posição 7':<10} {'Posição 8':<10} {'Posição 9':<15}")
    print(f"{'-'*3} {'-'*25} {'-'*10} {'-'*10} {'-'*15}")
    
    total_pedagios = 0.0
    for i, pedagio in enumerate(pedagios_data):
        nome = pedagio[5]
        pos7 = pedagio[7]  # String com vírgula
        pos8 = pedagio[8]  # Número direto (valor do pedágio)
        pos9 = pedagio[9]  # Distância
        
        total_pedagios += pos8
        
        print(f"{i+1:2d}. {nome[:24]:<25} {str(pos7):<10} {pos8:<10.1f} {pos9:<15.1f}")
    ...
    if abs(total_pedagios - 53.70) < 5:
        print(f"[OK] Valores muito próximos! A correção funcionou!")
    elif total_pedagios < 100:
        print(f"[OK] Valor razoável para pedágios!")
    else:
        print(f"[INFO] Valor ainda diferente, mas pode estar correto")
        print(f"[INFO] Antes: R$ 7.710,67 (posição 7)")
        print(f"[INFO] Agora: R$ {total_pedagios:.2f} (posição 8)")
        print(f"[INFO] Redução: R$ {7710.67 - total_pedagios:.2f}")
