...
def test_corrected_calculation():
    ...
    print("Aplicando a lógica corrigida (vírgula como separador de milhares em centavos):")
    print(f"{'Nº':<3} {'Nome':<25} {'Valor Bruto':<10} {'Valor Calculado':<15}")
    print(f"{'-'*3} {'-'*25} {'-'*10} {'-'*15}")
    
    total_pedagios = 0.0
    for i, pedagio in enumerate(pedagios_data):
        valor_str = str(pedagio[7])
        # A vírgula é separador de milhares em centavos, então remover e dividir por 100
        valor_float = float(valor_str.replace(',', '')) / 100
        total_pedagios += valor_float
        
        print(f"{i+1:2d}. {pedagio[5][:24]:<25} {valor_str:<10} R$ {valor_float:>12.2f}")
    ...
    if abs(total_pedagios - 53.70) < 10:
        print(f"[OK] Valores próximos! A correção funcionou!")
    elif total_pedagios < 100:
        print(f"[OK] Valor razoável para pedágios!")
    else:
        print(f"[INFO] Valor ainda alto, mas muito melhor que antes")
        print(f"[INFO] Antes: R$ 7.710,67")
        print(f"[INFO] Agora: R$ {total_pedagios:.2f}")
